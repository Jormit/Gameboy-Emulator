...
// CPU Operations
void cpu_cycle(); // Reads current opcode then executes instruction. Also prints
                  // output.
...
void render_tile_map(); // Arranges tiles according to tilemap and displays onto
                        // screen.
...
void set_lcd_status(); // Sets the lcd status register [0xFF41] according to the
                       // current scanline.
...
// Array of structures that uses instruction opcode as index and stores name
// length and function pointer.
...
void key_press(int key) {
  ...
  if (request_interupt && !previouslyUnset)
    set_interupt(4);
}

void key_release(int key) { joypad_state = Set(key, joypad_state); }
...
void cpu_cycle() {
  ...
  if (instructions[opcode].length == 0) {
    ...
    Operand16 =
        (read_byte(registers.pc + 2) << 8) + read_byte(registers.pc + 1);
    registers.pc += 3;
    ((void (*)(void))instructions[opcode].fcnPtr)();
  }
...
void add_byte(uint8_t Value2) {
  ...
  if ((((registers.a & 0xf) + (Value2 & 0xf)) > 0x0f)) {
    Set_H_Flag();
  } // Set Half Carry Flag.
  else {
    Clear_H_Flag();
...
void adc(uint8_t a) {
  ...
  if (((result & 0x0f) + (registers.a & 0x0f)) > 0x0f) {
    Set_H_Flag();
  } // Check Half Carry.
  else {
    Clear_H_Flag();
...
{
  {
    ...
    if (test_bit(6, registers.f)) {
      if (test_bit(5, registers.f))
        s = (s - 0x06) & 0xFF;
      if (test_bit(4, registers.f))
        s -= 0x60;
    } else {
      if (test_bit(5, registers.f) || (s & 0xF) > 9)
        s += 0x06;
      if (test_bit(4, registers.f) || s > 0x9F)
        s += 0x60;
    }
